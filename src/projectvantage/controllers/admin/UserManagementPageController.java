...
public class UserManagementPageController implements Initializable {
    ...
    private void loadTableData() {
        ...
        String sql = "SELECT user.id, first_name, middle_name, last_name, email, phone_number, username, salt, password, secret_key, role.name AS role, status.name AS status "
                + "FROM user INNER JOIN role ON user.role_id = role.id INNER JOIN status ON user.status_id = status.id";
